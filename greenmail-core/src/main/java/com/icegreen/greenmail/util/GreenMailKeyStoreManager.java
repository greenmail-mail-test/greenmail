...
public class GreenMailKeyStoreManager {
    protected static final Logger log = LoggerFactory.getLogger(GreenMailKeyStoreManager.class);
    ...
    public GreenMailKeyStoreManager() {
		try {
        ...
        try (InputStream is = Thread.currentThread().getContextClassLoader().getResourceAsStream(GREENMAIL_JKS)) {
            this.ks.load(is, pass);
            this.km.init(this.ks, pass);            
            ...
            log.warn("Can not load greenmail keystore from '" + GREENMAIL_JKS +
                    "' in classpath. Falling back to hard coded keystore.", e);
            e.printStackTrace();
			try {
				this.ks.load(new ByteArrayInputStream(HARD_CODED_KEY_STORE), pass);
		        this.km.init(this.ks, pass);
			} catch (NoSuchAlgorithmException | CertificateException | IOException | UnrecoverableKeyException | KeyStoreException e1) {
	            log.warn("Can not load greenmail keystore from  hard coded keystore.", e);
				e1.printStackTrace();
			}
        }
    }
